...
func setupThanosRoutes(router *gin.RouterGroup, server *servers.Server) {
	...
	router.POST("/:id/plugins/bridge", handler.InstallBridge)
	router.POST("/:id/plugins/block-explorer", handler.InstallBlockExplorer)
	router.DELETE("/:id/plugins/bridge", handler.UninstallBridge)
	router.DELETE("/:id/plugins/block-explorer", handler.UninstallBlockExplorer)
